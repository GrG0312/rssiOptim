...
namespace VLib.Args.Binding
{
    ...
    public sealed class OptionDescriptor
    {
        ...
        public void SetFromText(object target, string text)
        {
            ...
            if (!result.Success)
            {
                throw new VLibArgException($"Érvénytelen érték a(z) '{Name}' paraméterhez: {result.Error}", SuggestionHint(result.Error));
            }
            ...
            {
                throw new VLibArgException($"a(z) '{Name}' nem lehet üres.");
            }
        ...
        private string? SuggestionHint(string? error)
        {
            ...
            return $"Lehetséges értékek: {string.Join(", ", Parser.Suggestions)}";
        }
